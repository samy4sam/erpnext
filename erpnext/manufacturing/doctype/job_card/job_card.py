...
@frappe.whitelist()
def is_next_in_sequence(job_card):

	doc = frappe.get_doc("Job Card", job_card)
	if doc.is_new():
		return {"is_next_in_sequence": True}
	...
	current_operation_qty = 0.0
	data = doc.get_current_operation_data()
	if data and len(data) > 0:
		current_operation_qty = flt(data[0].completed_qty)

	current_operation_qty += flt(doc.total_completed_qty)

	data = frappe.get_all(
		"Work Order Operation",
		fields=["operation", "status", "completed_qty", "sequence_id", "idx"],
		filters={"docstatus": 1, "parent": doc.work_order, "status": "pending"},
		order_by="sequence_id, idx",
	)

	if data[0].operation == doc.operation:
		return {"is_next_in_sequence": True}
	else:
		return {"is_next_in_sequence": False}
