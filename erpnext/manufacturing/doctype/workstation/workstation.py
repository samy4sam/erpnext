...
def get_job_cards(workstation, job_card=None):
	if frappe.has_permission("Job Card", "read"):
		...
		for row in jc_data:
			if row.drawing:
				drawing_name = frappe.get_all(
					"File",
					fields=["name"],
					filters={
						"file_name": os.path.basename(row.drawing),
						"attached_to_name": row.name,
						"attached_to_field": "drawing",
					},
				)[0].name

				row.drawing_link = get_link_to_form("File", drawing_name, label="Schnittzeichnung")
			...
			row.raw_materials = get_raw_materials(row.name)
			row.make_material_request = False
			if row.for_quantity > row.transferred_qty or allow_excess_transfer or row.raw_materials:
