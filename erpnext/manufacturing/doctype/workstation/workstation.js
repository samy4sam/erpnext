...
class WorkstationDashboard {
	...
	setup_menu_actions() {
		...
		this.job_cards.forEach((data) => {
			frappe.call({
				method: "erpnext.manufacturing.doctype.job_card.job_card.is_next_in_sequence",
				args: {
					job_card: data.name,
				},
				callback: (r) => {
					let is_next_in_sequence = r.message.is_next_in_sequence;
					let menu_actions = me.$wrapper.find(`.menu-actions[data-job-card='${data.name}']`);
					let material_request_needed = false;
					
					data.raw_materials.forEach((row) => {
						if (row.required_qty > row.transferred_qty) {
							material_request_needed = true;
						}
					});
	
					$(menu_actions).find(".btn-start").hide();
					$(menu_actions).find(".btn-resume").hide();
					$(menu_actions).find(".btn-pause").hide();
					$(menu_actions).find(".btn-complete").hide();
					$(menu_actions).find(".btn-transfer-materials").hide();
					$(menu_actions).closest(".menu-btn-group").addClass('hidden').hide();

					if (is_next_in_sequence) {
						if (!material_request_needed &&
							...
							if (!data.time_logs?.length) {
								$(menu_actions).find(".btn-start").show();
							} else if (data.is_paused) {
								$(menu_actions).find(".btn-resume").show();
							} else if (data.for_quantity - data.manufactured_qty > 0) {
								if (!data.is_paused) {
									$(menu_actions).find(".btn-pause").show();
								}
								$(menu_actions).find(".btn-complete").show();
							}
						} else if (!data.skip_material_transfer || material_request_needed) {
							$(menu_actions).find(".btn-transfer-materials").show();
						}
						$(menu_actions).closest(".menu-btn-group").removeClass('hidden').show();;
					}
				}
			});
		});
